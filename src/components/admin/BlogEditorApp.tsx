...
export default function BlogEditorApp({ post, services }: Props) {
  const [fields, setFields] = useState({ title: post.title, slug: post.slug, excerpt: post.excerpt, category: post.category || 'عام', author: post.author || 'فريق فال', coverUrl: post.coverUrl, coverAlt: post.coverAlt, coverAssetId: post.coverAssetId, relatedServiceId: post.relatedServiceId });
